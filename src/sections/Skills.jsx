  ...
  return (
    <section id="skills" className="relative min-h-screen py-24 px-4 z-10">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        ...
        <div className="absolute bottom-20 right-10 w-96 h-96 bg-white/[0.03] rounded-full blur-[100px]" />
      </div>
      ...
      <div className="relative z-20 max-w-7xl mx-auto">
        ...
        <div className="grid lg:grid-cols-2 gap-8 mb-16">
          {skillCategories.map((category, categoryIndex) => (
            <div
              key={categoryIndex}
              className="bg-black/30 backdrop-blur-sm p-6 rounded-2xl border border-white/10 hover:border-white/20 transition-all duration-300"
            >
              ...
              <div className="flex flex-wrap gap-3">
                {category.skills.map((skill, skillIndex) => (
                  <span
                    key={skillIndex}
                    className="px-4 py-2 rounded-full text-sm font-medium text-slate-300 bg-white/5 border border-white/10 hover:border-white/30 hover:text-white transition-colors duration-300"
                  >
                    {skill}
                  </span>
                ))}
